...
impl CanvasFast {
    ...
    /// Wu's anti-aliased 1px line (fast pipeline, u32 pixels)
    fn draw_line_wu(
        &mut self,
        start: CircleF4E4,
        end: CircleF4E4,
    ...
    /// SDF-based thick line: single pass, no overlap, AA fringe.
    ///
    /// Per-endpoint cap styles: each end can independently be butt, round, or square.
    fn draw_line_sdf(
        &mut self,
        start: CircleF4E4,
        end: CircleF4E4,
        ...
        let r_px = (weight * self.coords.span * self.coords.ru * half).to_isize().max(1);
        ...
        for py in min_y..=max_y {
            ...
            for px in min_x..=max_x {
                ...
                let dist = if t.is_negative() {
                    ...
                    // Within segment — perpendicular distance
                ...
                self.pixels[idx] = CanvasFast::blend(colour, self.pixels[idx], alpha);
            }
        }
    }
}
